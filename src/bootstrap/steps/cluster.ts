...
export async function setupWorker(workerNumber, amountOfWorkers) {
	...
	const siteConfiguration: any[] = config.get("sites");
	const sites = siteConfiguration.map(configuredSite => {
		const site = { ...configuredSite };

		const concurrencyPerWorker = Math.floor(
			configuredSite.concurrency / amountOfWorkers
		);
		const totalDistributedConcurrency =
			concurrencyPerWorker * amountOfWorkers;
		const concurrencyFix =
			configuredSite.concurrency - totalDistributedConcurrency;
